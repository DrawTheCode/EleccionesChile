...
function makeATable (divId,data,config=null){
  ...
  if(nuloEmitidos && blcoEmitidos && valiEmitidos && apruEmitidos  && rchzEmitidos){
    ...
    tabla.innerHTML = '<tr><th>Opción</th><th>Total vistas</th><th>Total Porcentaje</th></tr>';
    tabla.appendChild(makeTrItem('',validSumApReEmitidos,`${percentValue(validSumApReEmitidos,valiEmitidos)}%`));
    tabla.appendChild(makeTrItem('A Favor',apruEmitidos,`${percentValue(apruEmitidos,valiEmitidos)}%`));
    tabla.appendChild(makeTrItem('En Contra',rchzEmitidos,`${percentValue(rchzEmitidos,valiEmitidos)}%`));
    tabla.appendChild(makeTrItem('Validamente Emitidos',valiEmitidos,`${percentValue(valiEmitidos,totalVotos)}%`));
    tabla.appendChild(makeTrItem('Votos Nulos',nuloEmitidos,`${percentValue(nuloEmitidos,totalVotos)}%`));
    tabla.appendChild(makeTrItem('Votos En Blanco',blcoEmitidos,`${percentValue(blcoEmitidos,totalVotos)}%`));
    tabla.appendChild(makeTrItem('Total de Votación',totalVotos,`${percentValue(totalVotos,totalVotos)}%`));
...
export const infoVotaciones = (totalMesas,percent,hour) => {
  const firstLine = totalMesas && percent ?`<li>${totalMesas * (percent/100) } mesas escrutadas de un total de ${totalMesas}, correspondiente al ${percent}%.</li>`:null;
  ...
  if(data.length>0){
    ...
    data.forEach(element => {
      if(!exclude || element.COD_ZONA!==exclude){
        if(element.COD_AMBITO==='1' && element.AMBITO==='5'){nuloEmitidos+=parseInt(element.VOTOS)}
        if(element.COD_AMBITO==='1' && element.AMBITO==='6'){blcoEmitidos+=parseInt(element.VOTOS)}
        if(element.COD_AMBITO==='1' && element.AMBITO==='7'){valiEmitidos+=parseInt(element.VOTOS)}
        if(element.COD_AMBITO==='100001' && element.AMBITO==='4'){apruEmitidos+=parseInt(element.VOTOS)}
        if(element.COD_AMBITO==='100002' && element.AMBITO==='4'){rchzEmitidos+=parseInt(element.VOTOS)}
      }
    });
    return{
      nulo:nuloEmitidos,
