    ...
    fetch(url)
      ...
      .then(data => {
        if (data && data.data && data.data.length > 0) {
          ...
          let result = {
            info: data.details,
            data: makeResult(data.data, exclude),
          };
          ...
          let showHour =
            config.header.showHour === true ? result.info.hour : false;
...
const configMainNac = {
  url: `https://apiservel.latercera.com/api/search/${elecID}/by/19001g`,
  header: {
...
const configNac = {
  url: `https://apiservel.latercera.com/api/search/${elecID}/by/8056p`,
  header: {
    txt: '<h3>Nacional</h3>',
    totalMesas: 39390,
  },
...
const configExtr = {
  url: `https://apiservel.latercera.com/api/search/${elecID}/by/type/p`,
  header: {
...
const makeConfigData = (elecID, idZone, zoneName, qtyMesas, type) => {
  ...
  return {
    url: `https://apiservel.latercera.com/api/search/${elecID}/by/${idZone}${type}`,
    header: {
