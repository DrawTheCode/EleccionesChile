import { infoVotaciones, disclaimlText, infoBarResults, makeGenericStructure } from './genericFunctions.js';
...
const globalResults = await getData('https://apiservel.latercera.com/api/search/by/type/g');
const countryResults = await getData('https://apiservel.latercera.com/api/search/by/type/p');
const continentResult = await getData('https://apiservel.latercera.com/api/search/by/type/n');
const regionResult = await getData('https://apiservel.latercera.com/api/search/by/type/q');
...
const makeDisplayFetch = async (divID,config) => {
  if(config.type && config.type.match(/^(g|p|n|q)$/)!==null){
    ...
    let tempResult = {info:await results.details,data:await{...extractData}};
